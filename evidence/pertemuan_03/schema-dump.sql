...
--
-- Host: localhost
-- Generation Time: Sep 10, 2026 at 07:28 AM
...
CREATE TABLE `tenants` (
  ...
  `status` enum('pending','active','suspended','inactive') NOT NULL,
...
ALTER TABLE `canteens`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `canteens_code_unique` (`code`);
...
ALTER TABLE `menus`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `menus_tenant_id_id_unique` (`tenant_id`,`id`);
...
ALTER TABLE `orders`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `orders_checkout_idempotency_key_unique` (`checkout_idempotency_key`);
...
ALTER TABLE `tenants`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `tenants_canteen_id_code_unique` (`canteen_id`,`code`),
  ADD UNIQUE KEY `tenants_id_canteen_id_unique` (`id`,`canteen_id`);
...
--
ALTER TABLE `menus`
  ADD CONSTRAINT `menus_tenant_id_foreign` FOREIGN KEY (`tenant_id`) REFERENCES `tenants` (`id`);
...
--
ALTER TABLE `tenants`
  ADD CONSTRAINT `tenants_canteen_id_foreign` FOREIGN KEY (`canteen_id`) REFERENCES `canteens` (`id`);
COMMIT;
